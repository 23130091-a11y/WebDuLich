...
document.querySelectorAll('.explore-btn').forEach((btn) => {
    btn.addEventListener('click', (e) => {
        ...
        modal.querySelector('.close-icon').addEventListener('click', () => modal.remove());
    });
});
...
// Đăng nhập bằng Google/Facebook (placeholder)
...
if (facebookLoginBtn) {
    facebookLoginBtn.addEventListener('click', () => {
        showAlert('Tính năng đăng nhập Facebook đang được phát triển', 'warning');
    });
}
...
// Bỏ qua chọn sở thích
const skipPreferencesBtn = document.getElementById('skipPreferencesBtn');
...
if (registerBtn) {
    registerBtn.addEventListener('click', async (e) => {
        ...
        if (!agreeTerms) {
            showAlert('Vui lòng đồng ý với điều khoản sử dụng');
            return;
        }
...
function checkAuthStatus() {
    ...
    if (user) {
        try {
            ...
            if (userDisplayName) userDisplayName.textContent = userData.username || userData.email.split('@')[0];
