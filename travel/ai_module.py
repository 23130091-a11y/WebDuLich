...
def analyze_sentiment(text):
    """
    Phân tích cảm xúc sử dụng AI (PhoBERT)
    
    Tự động fallback về rule-based nếu AI model không khả dụng
    ...
    Ví dụ:
        ...
        "Không tệ, khá ổn" -> score > 0 (AI hiểu phủ định)
        "Đắt nhưng xứng đáng" -> score > 0 (AI hiểu ngữ cảnh)
    """
    if not text:
        return 0.0, [], []
    
    try:
        # Thử dùng AI sentiment analysis
        from .ai_sentiment import analyze_sentiment_ai
        return analyze_sentiment_ai(text)
    except Exception as e:
        # Fallback về rule-based nếu AI không hoạt động
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"AI sentiment analysis failed, using rule-based: {e}")
        
        # Rule-based analysis (code cũ)
        ...
        import math
        if total_score != 0:
            sentiment_score = math.tanh(total_score / 3)
        else:
            sentiment_score = 0.0
...
def calculate_destination_score(destination):
    ...
    Note: Nên gọi với prefetch_related('reviews') để tránh N+1
    """
    from .models import Review, RecommendationScore
    
    # Try to use prefetched reviews first (if available)
    if hasattr(destination, '_prefetched_objects_cache') and 'reviews' in destination._prefetched_objects_cache:
        ...
        # Fallback: fetch reviews (only needed fields)
        reviews = list(Review.objects.filter(destination=destination).only('rating', 'sentiment_score'))
...
def recalculate_all_scores():
    ...
    Optimized: Sử dụng bulk operations để giảm queries
    """
    from .models import Destination, RecommendationScore
    from django.db.models import Avg, Count, Q
    
    # Optimize: prefetch reviews to avoid N+1
    ...
    if scores_to_update:
        RecommendationScore.objects.bulk_update(
            scores_to_update,
            ['overall_score', 'review_score', 'sentiment_score', 'popularity_score',
             'total_reviews', 'avg_rating', 'positive_review_ratio']
        )
    
    if scores_to_create:
        RecommendationScore.objects.bulk_create(scores_to_create)
...
def _calculate_score_data(destination):
    """
    Helper function to calculate score data without saving
    """
...
def search_destinations(query, filters=None):
    ...
    # Bắt đầu với tất cả địa điểm - optimize with select_related
    ...
    if query and query.strip():
        ...
        if not partial_match.exists():
            # Tìm kiếm thủ công với normalize - optimize: only fetch needed fields
            matching_ids = []
            for dest in Destination.objects.only('id', 'name', 'location', 'description').all():
            ...
            destinations = Destination.objects.select_related('recommendation').filter(id__in=matching_ids)
        else:
            # Kết hợp exact và partial
            all_ids = list(exact_match.values_list('id', flat=True)) + \
                     list(partial_match.values_list('id', flat=True))
            destinations = Destination.objects.select_related('recommendation').filter(id__in=all_ids)
    ...
    try:
        # Note: select_related already applied above, just annotate
